...
export const fetchPosts = async (): PostObject[] => {
  const res = await getRequest(`${WORDPRESS_API_URI}/posts`);
...
export const fetchPost = async (id: Id) => {
  const res = await getRequest(`${WORDPRESS_API_URI}/posts/${id}`);
  return await res.json();
};

export const fetchPostsWithTag = async (tag: string) => {
  const res = await getRequest(`${WORDPRESS_API_URI}/posts?tags=${tag}`);
  return await res.json();
};